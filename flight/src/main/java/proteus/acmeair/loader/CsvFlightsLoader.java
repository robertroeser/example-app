...
import proteus.acmeair.FlightServiceDelegate;
...
import static proteus.acmeair.loader.FlightsData.*;
...
public class CsvFlightsLoader implements FlightsLoader {
  ...
  static Airport newAirport(String airportCode, String airportName) {
    return new Airport(airportCode, airportName);
  }

  static Airport newAirport(String airportName) {
  ...
  private static boolean contains(List<Airport> airports, String code) {
  ...
  public FlightsData load(String name) {
    ...
    final UnicastProcessor<Flight> flightsFlux = UnicastProcessor.create();
    final UnicastProcessor<FlightSegment> flightSegmentsFlux = UnicastProcessor.create();
    final UnicastProcessor<Airport> airportsFlux = UnicastProcessor.create();
    ...
    try (LineNumberReader lnr =
      ...
      List<Airport> airports = readAirports(airportNames, airportCodes);
      ...
      while (line != null && !line.trim().isEmpty()) {
        ...
        if (!contains(airports, airportCode)) {
          Airport acm = newAirport(airportCode, airportName);
          airports.add(acm);
        }
        ...
        while (st.hasMoreTokens()) {
          ...
          flightSegmentsFlux.onNext(new FlightSegment(flightId, airportCode, toAirport, miles));
          for (int daysFromNow = -1; daysFromNow < MAX_FLIGHTS_PER_SEGMENT; daysFromNow++) {
            ...
            flightsFlux.onNext(
                new Flight(flightId, departureTime, arrivalTime, 500, 200, 10, 200, "B747"));
          }
          flightNumber++;
          indexIntoTopLine++;
        }
  ...
  private List<Airport> readAirports(String airportNames, String airportCodes) {
